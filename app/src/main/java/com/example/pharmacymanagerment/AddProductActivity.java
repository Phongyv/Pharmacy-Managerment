...
public class AddProductActivity extends AppCompatActivity {
    ...
    private EditText etProductName, etProductDescription, etProductPrice, etProductCategory;
    ...
    private void selectImage() {
        ImagePicker.with(this)
                .crop() // Cắt ảnh
                .compress(512) // Nén ảnh tối đa 512KB
                .maxResultSize(1080, 1080) // Giới hạn kích thước ảnh
                .galleryOnly() // Chỉ chọn từ thư viện (có thể bỏ nếu muốn cả camera)
                .createIntent(intent -> {
                    startActivityForResult(intent, 101); // Mã requestCode là 101
                    return null;
                });
    }
        ...
        db.collection("products").whereEqualTo("name", name).get()
                .addOnSuccessListener(queryDocumentSnapshots -> {
                    if (!queryDocumentSnapshots.isEmpty()) {
                        ...
                        uploadImageAndSaveProduct(name, description, price, category);
                    }
                })
    ...
    private void uploadImageAndSaveProduct(String name, String des, double price, String type) {
        String imageName = UUID.randomUUID().toString();
        StorageReference imageRef = storageRef.child(imageName);

        imageRef.putFile(imageUri)
                .addOnSuccessListener(taskSnapshot -> imageRef.getDownloadUrl().addOnSuccessListener(uri -> {
                    saveProductToFirestore(name, des, price, type, uri.toString());
                }))
                .addOnFailureListener(e -> {
                    progressDialog.dismiss();
    ...
    private void saveProductToFirestore(String name, String des, double price, String type, String img) {
